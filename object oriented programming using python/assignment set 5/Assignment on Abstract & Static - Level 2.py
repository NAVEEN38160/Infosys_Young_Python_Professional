...
class OccasionalCustomer(Customer):
    ...
    def calculate_bill_amount(self):
        ...
        if validation:
            tiffin_cost = 50
            travel_cost = 0
            if self.__distance_in_kms > 0 and self.__distance_in_kms < 3:
                travel_cost = 5*self.__distance_in_kms
            elif self.__distance_in_kms > 2 and self.__distance_in_kms < 6:
                travel_cost = self.__distance_in_kms*7.5
            self.bill_amount = tiffin_cost+travel_cost
...
class RegularCustomer(Customer):
    ...
    def calculate_bill_amount(self):
        validation = self.validate_no_of_tiffin()
        if validation:
            tiffin_cost = 50
            no_of_days = 7
            self.bill_amount = tiffin_cost*no_of_days*self.__no_of_tiffin
            return self.bill_amount
        else:
            self.bill_amount = -1
            return self.bill_amount
